export default function handler(req, res) {
  const { type = "Fixed", width = 1200, height = 1500 } = req.query;
  ...
  if (lowerType === 'sliding') {
    ...
    sashLine = `<line x1="${bx}" y1="${by + sh}" x2="${bx + sw}" y2="${by}" stroke="#333" stroke-width="2"/>`;
  } // Fixed window has no sash line
  ...
  const svg = `
    <svg xmlns="http://www.w3.org/2000/svg" width="400" height="500">
      ...
      <text x="${bx}" y="${by + sh + 25}" font-size="14" font-weight="bold">${type} Window</text>
    </svg>
  `;
